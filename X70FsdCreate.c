    ...
    )
{
    ...
    if (IsMyFakeFcb(FltObjects->FileObject)
        || IsConcernedProcess(FltObjects, &Status, &ProcType)) {
...
BOOLEAN IsConcernedCreateOptions(__inout PFLT_CALLBACK_DATA Data)
{
    ...
    // 目录文件
    BOOLEAN DirectoryFile = BooleanFlagOn(Options, FILE_DIRECTORY_FILE);
    ...
    __in  BOOLEAN Network)
{
    ...
    if (Network) {
        ...
        // 网络文件因为oplock，只能只读，要想解决去参考rdbss.sys
    ...
    // 如果缓存写需要加上直接写入文件，否则 cccaniwrite 内部会导致等待 pagingio 产生死锁.
    SetFlag(Options, FILE_WRITE_THROUGH);
#endif // USE_CACHE_READWRITE
...
BOOLEAN IsNeedSelfFcb(__inout PFLT_CALLBACK_DATA Data, PFLT_FILE_NAME_INFORMATION * nameInfo, PNTSTATUS pStatus)
{
    ...
    if ((*nameInfo)->Extension.Length == 0 && (*nameInfo)->Share.Length != 0) {
        ...
        if ((*nameInfo)->Share.Length >= NAMED_PIPE_PREFIX_LENGTH) {
            ...
            if (NAMED_PIPE_PREFIX_LENGTH == RtlCompareMemory(Add2Ptr(ShareName.Buffer,
                ShareName.Length - NAMED_PIPE_PREFIX_LENGTH),
                NAMED_PIPE_PREFIX,
        ...
        if ((*nameInfo)->Share.Length >= MAIL_SLOT_PREFIX_LENGTH) {
            ...
            if (MAIL_SLOT_PREFIX_LENGTH == RtlCompareMemory(Add2Ptr(ShareName.Buffer,
                ShareName.Length - MAIL_SLOT_PREFIX_LENGTH),
                MAIL_SLOT_PREFIX,
