...
PVOID
X70FsdMapUserBuffer(__inout PFLT_CALLBACK_DATA Data);
...
X70FsdSetTopLevelIrp(
    ...
    IN BOOLEAN ForceTopLevel,   // read Ê±ºòÎª true, true
    IN BOOLEAN SetTopLevel
    );

VOID
X70FsdFinishIoAtEof(IN PFSRTL_ADVANCED_FCB_HEADER Header);
...
VOID
X70FsdDeleteIrpContext(IN OUT PIRP_CONTEXT *IrpContext);
...
NTSTATUS WriteFileHeader(
    PCFLT_RELATED_OBJECTS FltObjects,
    PFILE_OBJECT FileObject,
    PLARGE_INTEGER RealFileSize,
    PUCHAR ProcessGuid,
    PUNICODE_STRING FileFullName
);
...
BOOLEAN X70FsdIsIrpTopLevel(IN PFLT_CALLBACK_DATA Data);
